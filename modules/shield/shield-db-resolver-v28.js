...
(() => {
  ...
  function findBrand(rawLow) {
    for (const b of BRANDS) {
      if (rawLow.includes(norm(b))) return b;
    }
    return "";
  }
  ...
  function resolve(spec, opts = {}) {
    ...
    const parsed = rows.map(r => ({ item: r, dev: parseDevice(r.name) }));
  ...
  window.ShieldDbResolverV28 = { parseDevice, normalizeLeak, resolve, readMaterials, latinize };
})();
