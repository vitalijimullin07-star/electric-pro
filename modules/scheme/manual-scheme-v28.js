...
(() => {
  ...
  function load() { try { const m = JSON.parse(localStorage.getItem(KEY) || "null"); if (m && m.input && Array.isArray(m.groups)) return m; } catch (e) {} return def(); }
  ...
  function buildTree() {
    ...
    const groupNodes = m.groups.map(g => {
      qd++;
      const autom = (g.lines || []).map(l => { qf++; return { id: "QF" + qf, type: "mcb", label: l.name || "Линия", rating: l.nom || "", children: [] }; });
      return { id: "QD" + qd, type: g.kind === "dif" ? "rcbo" : "rcd", label: (g.title || "Группа") + " " + (g.leak || 30) + "мА", rating: "40А/" + (g.leak || 30) + "мА", children: autom };
    });
    let rt, tail;
    if (inlineA.length) {
      const r = inlineA[0];
  ...
  function renderPreview() {
    ...
    if (!tree || !window.ShieldSchemeSVG) { svg.innerHTML = ""; svg.setAttribute("width", "300"); svg.setAttribute("height", "60"); return; }
    try { window.ShieldSchemeSVG.render(svg, tree); } catch (e) { svg.innerHTML = ""; }
  }
  ...
  function groupCard(g, gi) {
    const lines = (g.lines || []).map((l, li) => `
      <div class="ms-line">
        ...
        <select class="ms-line-nom" data-lnom="${gi}:${li}">${NOMS.map(n => opt(n, l.nom || "C16")).join("")}</select>
  ...
  function render() {
    ...
    root.innerHTML = `
      <div class="ms-wrap">
        ...
        <div class="ms-card">
          <div class="ms-sub">Предпросмотр однолинейки</div>
  ...
  function onInput(e) {
    ...
    if ((el = t.closest("[data-lnom]"))) { const [gi, li] = el.dataset.lnom.split(":").map(Number); if (m.groups[gi] && m.groups[gi].lines[li]) m.groups[gi].lines[li].nom = t.value; save(m); renderPreview(); return; }
  }
  ...
  function onClick(e) {
    ...
    if ((el = t.closest("[data-addline]"))) { const gi = +el.dataset.addline; if (m.groups[gi]) { (m.groups[gi].lines = m.groups[gi].lines || []).push({ id: uid(), name: "", nom: "C16" }); save(m); render(); } return; }
    ...
    if ((el = t.closest("[data-addgroup]"))) { m.groups.push({ id: uid(), title: "Группа " + (m.groups.length + 1), kind: "uzo", leak: 30, lines: [] }); save(m); render(); return; }
  ...
  function injectCss() {
    ...
    st.textContent = `
      ...
      .ms-sub{font:800 12px/1 system-ui;color:#475569;text-transform:uppercase;letter-spacing:.03em;margin-bottom:10px}
