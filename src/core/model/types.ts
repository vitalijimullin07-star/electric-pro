...
export interface Firmware {
  ...
  wasm?: string;
  /**
   * Другие платы с прошивками (по обозначению на схеме): например, пульт HG1 на ESP32-S3
   * с экраном — ядро его интерфейса в WebAssembly (base64), связь с контроллером по UART.
   */
  modules?: Record<string, { name: string; wasm: string }>;
}
