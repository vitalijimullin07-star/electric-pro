 ...
 * Вся логика — в ядре (vac_core.c, vac_ui.c, vac_drv.c), здесь только железо:
 * выводы, таймер 100 мкс, прерывание детектора нуля, АЦП, две шины I²C, зуммер,
 * настройки во флеше и страница управления по Wi-Fi (точка доступа «Pylesos»).
 *
...
<button onclick="c('auto')">Авто</button><button onclick="c('manual')">Ручной</button>
<button onclick="c('purge')">Продувка</button></div>
<p>Мощность: <input type="range" min="30" max="100" step="5" id="p" onchange="c('power '+this.value)"> <b id="pv"></b></p>
<table id="t"></table><p class="bad" id="f"></p>
<script>
const L={state:'Включён',mode:'Авто',running:'Турбины',p1:'Т1, %',p2:'Т2, %',i1:'Ток Т1, А',i2:'Ток Т2, А',tool:'Инструмент, А',
t1:'Т1, °C',t2:'Т2, °C',mains:'Сеть, В',vacuum:'Разрежение, кПа',flow:'Расход, м³/ч',speed:'Скорость, м/с',filter:'Фильтр, Па',runon:'Выбег, с'};
