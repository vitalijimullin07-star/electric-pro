...
function migrate(){
  if(localStorage.getItem(K.mig)!=="1"){
    ...
    if(!localStorage.getItem(K.my))wj(K.my,my.map(x=>item(x,x.type,"my")));
    if(!localStorage.getItem(K.srv))wj(K.srv,srv.length?srv.map(x=>item(x,x.type,"server")):seed());
...
function migrateNoSort(){if(!localStorage.getItem(K.my))wj(K.my,[]);if(!localStorage.getItem(K.srv))wj(K.srv,seed());if(!localStorage.getItem(K.mas))wj(K.mas,{})}
function save(b,rows){if(b==="server"&&!admin())return alert("БД сервера редактирует только админ."),false;if(b==="masters")return alert("БД мастеров только просмотр/копирование."),false;let key=b==="server"?K.srv:K.my;wj(key,sortRows(rows.map(x=>item(x,x.type,b))));localStorage.setItem(K.save,new Date().toISOString());log("save","БД сохранена локально",{base:b,count:rows.length});return true}
...
${can()?`<button data-db26-add>Добавить позицию</button><button data-db26-move>Переместить</button><button data-db26-del>Удалить</button>`:""}
...
function externalDbTrigger(x){if(!x||x.closest?.("#ep-db-v26,#db26modal"))return false;let t=nrm(x.textContent),a=[x.id,x.className,x.getAttribute?.("data-route"),x.getAttribute?.("data-screen"),x.getAttribute?.("data-ep-shell-route"),x.getAttribute?.("onclick"),x.getAttribute?.("href"),x.getAttribute?.("aria-label")].filter(Boolean).join(" ").toLowerCase();return a.includes("database")||(t.includes("база данных")&&t.length<180)}
...
function click(e){
  ...
  if(e.target.closest("[data-db264-autosort]"))return e.preventDefault(),autoSortStorage(true),render();
}
function change(e){
  if(!e.target.closest?.("#ep-db-v26"))return;
