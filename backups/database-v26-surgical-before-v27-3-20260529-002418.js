...
function externalDbTrigger(x){if(!x||x.closest?.("#ep-db-v26,#db26modal,.ep2615-picker,.ep27-picker"))return false;if(x.closest?.("[data-ep2615-main-picker],[data-ep2616-main-picker],[data-ep27-db-picker],[data-ep27-pick],.ep2615-estimate-card,.ep27-estimate-card"))return false;let t=nrm(x.textContent),a=[x.id,x.className,x.getAttribute?.("data-route"),x.getAttribute?.("data-screen"),x.getAttribute?.("data-ep-shell-route"),x.getAttribute?.("onclick"),x.getAttribute?.("href"),x.getAttribute?.("aria-label")].filter(Boolean).join(" ").toLowerCase();return a.includes("database")||(t.includes("база данных")&&t.length<180)}
